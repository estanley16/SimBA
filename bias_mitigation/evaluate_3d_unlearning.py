...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 21 13:02:57 2023

@author: emma

evaluation code for unlearning experiments (loads and concatenates model for disease prediction)
"""
...
def compute_metrics(df, save_dir, label):
    ...
    # plt.savefig(save_dir + label + '_bal_cm.png', bbox_inches='tight', dpi = 800)
    # plt.show()
    plt.clf()
...
def main():
    ...
    # fn_test = working_dir + "test_bal.csv"
    ...
    #encoder
    encoder = tf.keras.models.load_model(working_dir + 'best_un_encoder_'+exp_name+'.h5')

    #disease classification head
    disease_head = tf.keras.models.load_model(working_dir + 'best_un_D_classifier_'+exp_name+'.h5')

    

    d_model = disease_head(encoder.output)
    model = tf.keras.models.Model(inputs=encoder.input, outputs = d_model)
