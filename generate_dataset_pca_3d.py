...
code used for generating dataset in MICCAI 2023 paper: 
A flexible framework for simulating and evaluating biases in deep learning-based medical image analysis


for generating dataset with simulated disease, bias, and subject variability effects.
- running this script will create a subdirectory under "main_dir" with datasets containing the either the "disease" or "no disease" target effect (depending on input to --disease argument)
- if nonzero --bias_frac is specified, that fraction of datasets will contain the desired bias effect (indicated by suffix _B in the filename)
- includes inter-subject variability (isv) if --isv is set to 1

notes:
- this script assumes binary disease classes and bias groups
- this script samples effects from PCA models trained on velocity fields -- can be modified to use displacement/velocity fields from 
- the original script saved information about whether deformations are non-diffeomorphic but were removed from this version for readability. 
    velocity/displacement fields can be checked for diffeomorphism with function diffeomorphic_check() in utils.py
...
parser.add_argument('--n_samples', type=int, help='number of images to generate for this D/ND class', required=True)
parser.add_argument('--disease', type=int, help='0 for ND, 1 for D', required=True)
parser.add_argument('--bias_frac', type=float, help='proportion of D/ND class with bias features', required=True)
...
if args.disease==1:
    name = args.expname + '_D'
else:
    name = args.expname + '_ND'
    
#create directory for dataset
Path(main_dir + name).mkdir(parents=True, exist_ok=True)
...
# ------ disease effect ------
pca_d = utils.load_pca_model(pca_dir, 'sri24_spgr_RAI_lpba40_' + str(args.d_roi)) #open PCA model for disease effect#open PCA model for disease effect
#define D/ND sampling distribution 
if args.disease==1:
    effect_dst_mean = 2 #mean value of component for generating distribution of samples 
else:
    effect_dst_mean = 0 #mean value of component for generating distribution of samples  
effect_dst_sd = 1 #standard deviation for generating distirbution of samples 
effect_dst = np.random.normal(effect_dst_mean, effect_dst_sd, args.n_samples) #get samples from disease effect distribution 


# ------ bias effect ------
if args.bias_frac > 0: 
    pca_b = utils.load_pca_model(pca_dir, 'sri24_spgr_RAI_lpba40_' + str(args.b_roi)) #open PCA model for bias effect
num_b = int(round(args.bias_frac * args.n_samples)) # number of images that will contain the bias feature
...
bias_dst = np.random.normal(bias_dst_mean, bias_dst_sd, num_b) #get samples from bias effect distribution 
...
if args.isv == 1: 
    ...
    #define isv sampling distribution -> truncated, since sampling values outside of (-1.9. 1.9) results in non-diffeomorphic transformations
    #https://stackoverflow.com/questions/64601585/how-to-best-get-a-sample-from-a-truncated-normal-distribution
    isv_dst_mean = 0 #mean value of component for generating distribution of samples 
    isv_dst_sd = 1 #standard deviation for generating distirbution of samples 
    isv_dst_bounds = [-3, 3]
    isv_dst = truncnorm.rvs((isv_dst_bounds[0]-isv_dst_mean)/isv_dst_sd, (isv_dst_bounds[1]-isv_dst_mean)/isv_dst_sd, loc=isv_dst_mean, scale=isv_dst_sd, size=args.n_samples)
    # isv_dst = np.random.normal(isv_dst_mean, isv_dst_sd, args.n_samples) #standard gaussian dist sampling
...
for n in range(args.n_samples):
    ...
    #get label for image
    label = str(np.around(effect_dst[n],3)) + '_D'
    ...
    writer.SetFileName(main_dir + name + '/' + str(n).zfill(5) + '_' + label + '.nii.gz')
